...
int main()
{
	...
	while (!glfwWindowShouldClose(window) && fSimulationRunning)
	{
		...
		if( progress_counter > time_end ){
			// redis_client_graphics.set(POSE_COMPLETE_KEY, bool_to_string(true));
			cout << "POSE COMPETE";
			progress_counter = 0; 
			show_progress_bar = false;
		} else {
			redis_client_graphics.set(POSE_COMPLETE_KEY, bool_to_string(false));
		}
